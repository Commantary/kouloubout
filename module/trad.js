...
module.exports.run = async (client, message, args) => {
	...
	if (args[0] == undefined){
		return message.channel.send({embed: {
			color: 16406616,
			description: "Il faut préciser un nom de pokémon !"
		}});
	}
	...
	if (pokemon.getId(pokemonName, "fr") === -1){
		return message.channel.send({embed: {
			color: 16406616,
			description: "Ce nom de pokémon n'existe pas !"
		}});
	}
		...
		return message.channel.send({embed: {
			color: 15922601,
			description: "La traduction du pokémon **" + pokemonName + "** est **" + pokemon.getName(pokemonId) + "**"
		}});
